...
public class CountryRoute {
    ...
    @Bean
   @RouterOperations({
    @RouterOperation(path = "/countries", produces = MediaType.APPLICATION_JSON_VALUE, // Usa MediaType
        operation = @Operation(summary = "Obtener todos los países", description = "Retorna una lista de países",
            responses = { // Define las respuestas posibles
                @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Lista de países",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, // Especifica el tipo de contenido
                        schema = @Schema(implementation = CountryEntity.class))) // Define el modelo de datos
            }))
    @RouterOperation(path = "/countries/{id}", produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.GET,
        operation = @Operation(summary = "Obtener país por ID", description = "Retorna un país según su ID",
            parameters = { // Define los parámetros de la ruta
                @Parameter(name = "id", description = "ID del país", required = true) // Parámetro obligatorio
            },
            responses = {
                ...
                @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "País no encontrado") // Ejemplo de respuesta 404
            }))
    @RouterOperation(path = "/countries", produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.POST,
        operation = @Operation(summary = "Crear un nuevo país", description = "Guarda un nuevo país en la base de datos",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody( // Define el cuerpo de la petición
                content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
                    schema = @Schema(implementation = CountryEntity.class))), // Modelo de datos para la petición
            responses = {
    ...
    @RouterOperation(path = "/countries/{id}", produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.DELETE,
        operation = @Operation(summary = "Eliminar un país", description = "Borra un país según su ID",
            ...
            responses = {
                @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "204", description = "País eliminado") // Código 204 (No Content)
            }))
