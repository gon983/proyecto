...
public class PurchaseRouter {
    ...
    public RouterFunction<ServerResponse> purchaseRoutes(PurchaseService purchaseService, ConfirmPurchaseMiddleware confirmPurchaseMiddleware, UserContextService userContext) {
        return route(GET("/api/user/purchases/{idPurchase}"), request -> getPurchaseWithDetails(request, purchaseService))
                ...
                .andRoute(PUT("/api/admin/agregar-a-recorrido/{idPurchase}"), request -> putRecorrido(request, purchaseService))
                .andRoute(PUT("/api/admin/quitar-de-recorrido/{idPurchase}"), request -> unputRecorrido(request, purchaseService))
                .andRoute(GET("/api/admin/ver-ventas-recorrido/{idRecorrido}"), request -> getVentasRecorrido(request, purchaseService))
                .andRoute(GET("/api/admin/all-confirmed-purchases"), request -> getAllConfirmedPurchases(request, purchaseService))
                .andRoute(GET("/api/admin/all-finalized-purchases"), request -> getAllFinalizedPurchases(request, purchaseService))
    ...
    private Mono<ServerResponse> cerrarVentasDia(ServerRequest request, PurchaseService purchaseService){
        return purchaseService.cerrarVentasDia().then(ServerResponse.ok().build());
    }
    ...
    private Mono<ServerResponse> getAllFinalizedPurchases(ServerRequest request, PurchaseService purchaseService) {
        
        return purchaseService.getAllFinalizedPurchasesWithDetails()
                .collectList()
                .flatMap(list -> ServerResponse.ok().bodyValue(list))
                .switchIfEmpty(ServerResponse.notFound().build());
    }

    private Mono<ServerResponse> getVentasRecorrido(ServerRequest request, PurchaseService purchaseService) {
        UUID idRecorrido = UUID.fromString(request.pathVariable("idRecorrido"));
        return purchaseService.getAllConRecorridoPurchasesWithDetailsFromRecorrido(idRecorrido)
                .collectList()
                .flatMap(list -> ServerResponse.ok().bodyValue(list))
                .switchIfEmpty(ServerResponse.notFound().build());
    }
    ...
    private Mono<ServerResponse> unputRecorrido(ServerRequest request, PurchaseService purchaseService){
        UUID idPurchase = UUID.fromString(request.pathVariable("idPurchase"));
        return request.bodyToMono(AddRecorridoToPurchaseDTO.class)
                      .flatMap(dto -> purchaseService.unputRecorrido(idPurchase, dto).then(ServerResponse.ok().build()));
    }
