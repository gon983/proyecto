...
public interface CollectionPointHistoryRepository extends R2dbcRepository<CollectionPointHistoryEntity, UUID> {
    ...
    @Query("SELECT * FROM collectionpointhistory WHERE id_collection_point = :collectionPointId")
    Flux<CollectionPointHistoryEntity> findByCollectionPointId(UUID collectionPointId);

    @Query("INSERT INTO collectionpointhistory (id_collection_point_history, id_collection_point, id_collection_point_state, description, initial_date, final_date) " +
