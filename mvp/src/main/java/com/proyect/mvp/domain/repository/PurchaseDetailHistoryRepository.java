...
public interface PurchaseDetailHistoryRepository extends R2dbcRepository<PurchaseDetailHistoryEntity, UUID> {
    ...
    @Query("SELECT * FROM purchase_detail_history WHERE fk_purchase_detail = :purchaseDetailId ORDER BY initial_date ASC")
    Flux<PurchaseDetailHistoryEntity> findByPurchaseDetailId(UUID purchaseDetailId);

    @Query("INSERT INTO purchase_detail_history (id_purchase_detail_history, fk_purchase_detail, fk_purchase_detail_state, initial_date, final_date, changed_by) " +
