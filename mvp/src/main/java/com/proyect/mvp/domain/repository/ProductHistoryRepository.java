...
public interface ProductHistoryRepository extends R2dbcRepository<ProductHistoryEntity, UUID> {
    ...
    @Query("SELECT * FROM product_history WHERE id_product = :productId ORDER BY change_date ASC")
    Flux<ProductHistoryEntity> findByProductId(UUID productId);

    @Query("INSERT INTO product_history (id_product_history, id_product, id_product_state, previous_product_state, change_date, changed_by) " +
